...
websocket.onmessage = function (event) {
    ...
    switch (data.type) {
        case 'temp':
            // console.log(data)
            temp.innerHTML = data.data.slice(0, -1)
    }
};
...
rebootBtn.addEventListener('click', () => {
    websocket.send(JSON.stringify({ action: 'reboot', req: inputAuto.value }));
})
shutdownBtn.addEventListener('click', () => {
    websocket.send(JSON.stringify({ action: 'shutdown', req: inputAuto.value }));
})
